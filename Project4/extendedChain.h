...
void extendedChain<T>::merge(extendedChain<T> &a, extendedChain<T> &b) {
	...
	clear();
	//chainNode<T> *pa=a.firstNode, *pb=b.firstNode;
	...
	while (a.firstNode != NULL&&b.firstNode!= NULL) {
		if (a.firstNode->element <= b.firstNode->element) {
			...
			currentNode = a.firstNode;
			a.firstNode = a.firstNode->next;
		}
		else {
			currentNode->next = b.firstNode;
			currentNode = b.firstNode;
			b.firstNode = b.firstNode->next;
		}
	}
...
template<class T>
extendedChain<T> merge2(extendedChain<T> &a, extendedChain<T> &b) {
	int a_size = a.size(), b_size = b.size();
	if (a_size == 0)
		return b;
	if (b_size == 0)
		return a;
	static extendedChain<T> c;
	int i = 0, j = 0;
